...
const validateField = (field: FormField, value: any): string | null => {
  ...
  const renderField = (field: FormField) => {
    ...
    switch (type) {
      ...
      // case 'date':
      //   const [isOpen, setIsOpen] = useState(false);  // add popover open state
      
      //   const currentDate = formData[field.id] ? new Date(formData[field.id]) : new Date();
      
      //   const handleYearChange = (year: string) => {
      //     const updated = new Date(currentDate);
      //     updated.setFullYear(parseInt(year));
      //     handleFieldChange(field.id, updated.toISOString());
      //   };
      ...
      //         {field.required && <span className="text-destructive ml-1">*</span>}
      //       </Label>
      //       <Popover open={isOpen} onOpenChange={setIsOpen}>  {/* controlled popover */}
      //         <PopoverTrigger asChild>
      //           <Button
      //             id={field.id}
      //             variant="outline"
      ...
      //               error && 'border-destructive'
      //             )}
      //             onClick={() => setIsOpen(true)}  // explicitly open popover on button click
      //           >
      //             <CalendarIcon className="mr-2 h-4 w-4" />
      //             {formData[field.id]
      ...
      //             selected={formData[field.id] ? new Date(formData[field.id]) : undefined}
      //             onSelect={(date) => {
      //               handleFieldChange(field.id, date ? date.toISOString() : '');
