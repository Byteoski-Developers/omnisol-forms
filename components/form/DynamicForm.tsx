...
export function DynamicForm({
  ...
  const paramValue =
    typeof window !== "undefined" ? searchParams.get("param") : null;
  ...
  const evaluateCondition = useCallback(
    (condition: any): boolean => {
      ...
      if ("operator" in condition) {
        ...
        console.log(
          "Evaluating complex condition - operator:",
          operator,
          "conditions:",
          conditions
        );
      ...
      console.log(
        "Evaluating simple condition - field:",
        field,
        "value:",
        value,
        "not:",
  ...
  const visibleFieldsCalculation = useMemo(() => {
    ...
    form.fields.forEach((field) => {
      ...
      const dependenciesMet =
        !field.dependencies ||
        field.dependencies.every((dep) => formData[dep.fieldId] === dep.value);

      // Check showIf condition if it exists
      let showIfConditionMet = true;
      if (field.showIf) {
        console.log(
          "Checking showIf for field:",
          field.id,
          "showIf:",
          field.showIf
        );
        ...
        console.log(
          "Final condition result for field",
          field.id,
          ":",
          showIfConditionMet
        );
  ...
  const validateField = (field: FormField, value: any): string | null => {
    ...
    if (field.type === "date" && value) {
      ...
      if (
        travelFields.some((fieldName) =>
          field.id.toLowerCase().includes(fieldName.toLowerCase())
        )
      ) {
        ...
        if (
          field.id === "plannedDepartureDate" &&
          formData["plannedArrivalDate"]
        ) {
    ...
    if (field.validations) {
      ...
      if (
        field.validations.customValidation &&
        !field.validations.customValidation(value)
      ) {
        return "Invalid value";
      }
    }
  ...
  const saveAllChanges = async () => {
    ...
    const currentStepSlug =
      form.steps && form.steps[currentStep]
        ? form.steps[currentStep].slug
        : undefined;
    ...
    const questionsWithStringifiedAnswers = pendingChanges.map((change) => ({
      ...change,
      answer:
        typeof change.answer === "object" && change.answer !== null
  ...
  const renderField = (field: FormField) => {
    ...
    const commonProps = {
      ...
      required,
      description,
      handleChange: (val: any, save?: boolean) =>
        handleFieldChange(id, val, save),
      readonly: false,
    };
      ...
      case "siblingsInput":
        return (
          <div className="space-y-2" key={field.id}>
            ...
            <SiblingTabs
              value={
                formData[field.id] ? JSON.stringify(formData[field.id]) : ""
              }
          ...
          <div className="space-y-2" key={field.id}>
            ...
            <WorkExperience
              onChange={(experiences) =>
                handleFieldChange(field.id, experiences)
              }
              value={formData[field.id]}
            />
            {error && (
      ...
      case "educationHistory":
        return (
          <div className="space-y-2" key={field.id}>
            ...
            <EducationHistory
              onChange={(value, save) =>
                handleFieldChange(field.id, value, save)
              }
              value={formData[field.id]}
              readonly={false}
            />
      ...
      case "file":
        return (
          <div className="space-y-2 mb-6" key={field.id}>
            ...
            {!formData[field.id] ? (
              <div
                className="flex items-center justify-center h-32 bg-muted rounded-md cursor-pointer border border-dashed"
                onClick={() => {
            ...
            <TextArea
              ...
              setErrorSubmission={(hasError) => {
                if (hasError) {
                  setErrors((prev) => ({
                    ...prev,
                    [field.id]: "Character limit exceeded",
                  }));
                } else {
                  setErrors((prev) => {
                    const newErrors = { ...prev };
        ...
        return (
          <div key={field.id} className="mb-6">
            <CheckboxMultiSelect
              ...
              helpText={
                field.helpText ||
                "Select all sources that apply to your situation. This helps us determine the exact documents you'll need to provide."
              }
          ...
          <div key={field.id} className="mb-4">
            ...
            <SocialHandles
              ...
              handleChange={(handles) =>
                handleFieldChange(field.id, handles.value)
              }
            />
            {error && (
              <div className="flex items-center gap-2 text-destructive mt-2">
      ...
      case "last10YearActivity":
        return (
          <div key={field.id} className="mb-4">
            ...
            <Last10YearActivity
              inputValue={formData[field.id] || []}
              handleChange={(activities: any) =>
                handleFieldChange(field.id, activities.value)
              }
              readonly={false}
            />
            {error && (
        ...
        return (
          <div key={field.id} className="mb-4">
            ...
            <LanguageTestInput
              inputValue={formData[field.id] || {}}
              handleChange={(val: any, save?: boolean) =>
                handleFieldChange(field.id, val, save)
              }
              readonly={false}
            />
            {error && (
  ...
  const ensureAllVisibleFieldsAreQueued = () => {
    ...
    const missingFieldIds = visibleFieldIds.filter(
      (id) => !pendingFieldIds.includes(id)
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="flex flex-col md:flex-row md:space-x-6">
        ...
        <div
          className={
            currentStep !== 0 ? "md:w-2/3 space-y-6" : "w-full space-y-6"
          }
        >
